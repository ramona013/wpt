...
async def test_drag_and_drop_with_draggable_element(bidi_session, top_context,
    ...
    actions.add_pointer().pointer_move(
        x=0, y=0, origin=get_element_origin(drag_target)).pointer_down(
            button=0).pointer_move(
                x=0,
                y=0,
                duration=drag_duration,
                origin=get_element_origin(drop_target)).pointer_up(button=0)
    ...
    if drag_duration != 0:
        assert rindex(drag_events_captured,
                      "dragenter") < rindex(drag_events_captured, "dragleave")
        assert rindex(drag_events_captured,
                      "dragleave") < rindex(drag_events_captured, "dragover")
    else:
        assert rindex(drag_events_captured,
                      "dragenter") < rindex(drag_events_captured, "dragover")
    assert rindex(drag_events_captured,
